...
def get_file_sound(file_path):
    return parselmouth.Sound(file_path)

def get_file_duration(sound):
    return call(sound, "Get total duration")

def get_file_pitch(sound, f0min=75, f0max=300):
    return call(sound, "To Pitch", 0.0, f0min, f0max)

def get_file_mean_pitch(pitch, unit="Hertz"):
    return call(pitch, "Get mean", 0, 0, unit)

def get_file_pitch_stddev(pitch, unit="Hertz"):
    return call(pitch, "Get standard deviation", 0, 0, unit)

def get_file_hnr(sound, f0min=75):
    harmonicity = call(sound, "To Harmonicity (cc)", 0.01, f0min, 0.1, 1.0)
    return call(harmonicity, "Get mean", 0, 0)

def get_file_jitter(sound, f0min=75, f0max=300):
...
def get_file_shimmer(sound, f0min=75, f0max=300):
    point_process = call(sound, "To PointProcess (periodic, cc)", f0min, f0max)
